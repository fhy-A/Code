...
def sync_missing_bundled_skills(bundled_skills_dir, installed_skills_dir):
    ...
    for source in sources:
        ...
        temp_dir = Path(tempfile.mkdtemp(
            prefix=f".{name}.bundled-",
            dir=installed_skills_dir,
        ))
        try:
...
def delete_installed_skill(name, bundled_skills_dir, installed_skills_dir):
    ...
    backup = installed_skills_dir / f".{name}.delete-{uuid.uuid4().hex}"
    try:
    ...
    try:
        ...
        try:
            _atomic_write_state(installed_skills_dir, previous_state)
        except Exception as rollback_exc:
            rollback_errors.append(f"state: {rollback_exc}")
        detail = "; ".join(rollback_errors) if rollback_errors else "rolled back"
        raise BundledSkillStateError(
            f"bundled Skill delete failed ({detail}): {exc}",
        ) from exc
    return {"ok": True}
