...
> 状态：阶段 1 至 5 已完成；阶段 6 的 6A 至 6C 已完成，6D-1 已移除消息交互对内联事件和散落全局函数的依赖，后续兼容层继续分批收口
> 适用范围：Code 前端  
...
3. `v0.5.32` 已作为首个 bundle 默认入口正式版本发布，并通过 3010/3011、默认页面和经典回退人工验收；6D-1 只先收口消息交互的内联事件与散落全局函数，经典页面、原始脚本及 `window.Code` 模块命名空间继续保留。后续按独立阶段评估剩余代理和兼容资源，TypeScript 继续后置。
