...
> 状态：阶段 1 至 5 已完成；阶段 6 进行中（6A/6B/6C-1/6C-2 已完成，下一步为 6C-3 EXE 与发布链接入）
> 适用范围：Code 前端  
...
2. 阶段 6 的 6A 构建链、6B 显式预览、6C-1 哈希/回退护栏和 6C-2 开发默认入口已经完成；下一步 6C-3 只接入 EXE 与发布链，不继续扩大前端运行时改动。
3. 正式 EXE 当前仍使用普通脚本加载方式；6C-3 完成后，bundle 成为正式默认入口，但经典页面和原始脚本仍至少保留一个稳定正式版本。
