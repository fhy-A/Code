...
> 状态：阶段 1 至 5 已完成；阶段 6 esbuild 属于后置构建体系改造，需单独分析与确认
> 适用范围：Code 前端  
...
2. 阶段 6 esbuild 会改变资源加载、开发调试、正式 EXE 打包和回退方式，启动前必须重新进行只读分析、给出兼容与回退方案并获得用户确认。
3. 在阶段 6 启动前，现有普通脚本加载方式和 `window.Code` 模块边界继续作为稳定基线维护。
