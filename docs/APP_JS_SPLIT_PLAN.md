...
> 状态：阶段 1 至 5 已完成；阶段 6 的 6A 至 6C 已完成，6D-1/3 已收口消息交互、bundle 启动与 AgentRuntime 顶层全局，后续兼容层继续分批处理
> 适用范围：Code 前端  
...
2. 阶段 6 的 6A 至 6C 已完成；下一次正式发版需按现有发布流程生成首个 bundle 默认入口版本，并由用户验证更新、3010/3011 端口隔离、默认页面及经典回退，不从本地构建自动推断为已经发布。
3. `v0.5.32` 已作为首个 bundle 默认入口正式版本发布，并通过 3010/3011、默认页面和经典回退人工验收；6D-1/3 已收口消息交互、bundle 启动及 AgentRuntime 顶层全局，经典页面、原始脚本及 `window.Code` 模块命名空间继续保留。后续按独立阶段评估剩余代理和兼容资源，TypeScript 继续后置。
