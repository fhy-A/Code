...
> 状态：进行中（阶段 1 至 4、阶段 5A 至 5F、阶段 5G-1 已完成，下一步进行 5G-2 收口决策）
> 适用范围：Code 前端  
...
- [ ] 5G-2：根据审计结果决定是否创建 `agent-loop.js`。当前建议不创建：唯一可迁移候选只是单消费者的状态标签映射，无法隔离任何运行时副作用，跨文件接口成本高于维护收益；待定向与全量回归通过后按“保留编排层”完成阶段 5 收口。
...
阶段 5A 至 5F 已按职责收口，5G-1 已完成主循环状态与所有权审计。下一步进行 5G-2 收口决策：

1. 运行 Agent 主链定向回归与完整测试，确认新增状态矩阵护栏不遗漏既有恢复、授权、问卷、取消和失败语义。
2. 当前建议不创建 `agent-loop.js`：保留 `runServerAgentLoop()` 作为跨 AgentRuntime、网络、事件投影、问卷、授权和终态清理的入口编排层，并以源码护栏固定其状态顺序。
3. 若验证没有发现新的可复用纯边界，则标记 5G 与阶段 5 完成；不迁移只有一个消费者的状态标签映射，不以目录结构目标制造薄模块。
