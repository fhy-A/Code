...
> 状态：阶段 1 至 5 已完成；阶段 6 进行中（6A/6B/6C-1 已完成，下一步为 6C-2 开发默认入口切换）
> 适用范围：Code 前端  
...
- [ ] 6C-2：将开发默认入口切换到 bundle，增加 bundle 加载/入口初始化失败时的经典页面回退，并在 3011 启动及根页面刷新时按状态清单自动重建过期产物；完成默认页、源码修改后刷新和经典回退人工验收。
- [ ] 6C-3：把强制构建与新鲜度门禁接入 `build_exe.py` 和发布脚本；正式 EXE 同时携带默认 bundle、经典回退页面及原始脚本资源，不打包 source map/metafile，并验证构建失败或过期产物无法进入打包步骤。
- [ ] 6D：至少经过一个稳定正式版本后，再评估移除经典脚本兼容资源及 `window.Code` 中只用于过渡的代理；TypeScript 继续后置。
...
2. 阶段 6 的 6A 构建链、6B 显式预览和 6C-1 哈希/回退护栏已经完成；下一步 6C-2 只切换开发默认入口并接入开发期自动重建，人工验收后再进入 6C-3 EXE 与发布链。
3. 6C-2 完成前，现有普通脚本加载方式和正式 EXE 资源继续作为默认稳定基线；6C-3 完成后，经典页面和原始脚本仍至少保留一个稳定正式版本。
