...
> 状态：阶段 1 至 5 已完成；阶段 6 进行中（6A 构建链与兼容护栏已完成，下一步为 6B 显式预览入口）
> 适用范围：Code 前端  
...
- [ ] 6B：增加只在显式地址下使用的 3011 bundle 预览页面，默认根页面继续加载原始脚本；完成启动、对话、工具、刷新恢复、问卷、压缩、设置与中英文切换人工冒烟后，再决定是否进入 6C 默认入口切换。
- [ ] 6C：人工验收通过后切换默认入口，并把 bundle 新鲜度检查接入开发启动、`build_exe.py` 与发布脚本；首个正式版本同时保留原始脚本回退资源，构建失败或产物过期必须阻止打包。
...
2. 阶段 6 已按确认方案开始，6A 只建立旁路构建能力且未改变运行入口；下一步 6B 仅开放 3011 显式预览页，并以人工冒烟结果作为是否进入 6C 的门禁。
3. 在 6C 获得单独确认前，现有普通脚本加载方式、正式 EXE 资源和 `window.Code` 模块边界继续作为默认稳定基线维护。
