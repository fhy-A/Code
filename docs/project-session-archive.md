...
确认弹窗显示项目名称、服务端固定目标的完整会话名称和数量，活动工作仍须明确确认停止；名称由必要元数据读取，不依赖页面分页中的 state.sessions。确认提交即关闭弹窗，页面显示非阻塞处理反馈。成功仅提示实际数量及设置页恢复入口，不常驻已完成结果列表、内部操作 ID 或空的取消/重试按钮。失败、部分失败和未知结果保留可访问的原因与核对/继续/重新确认失败项入口，不自动弹回结果页。
...
归档分组显示总数；搜索时显示匹配数/总数，总数来自完整已加载归档集合，不把匹配数当作整组删除数量。当前项目、未归属和历史项目均适用，恢复/删除/刷新后同步更新。选中项目最后一项消失时先复位筛选再更新列表；同名项目或多个已移除项目用稳定 ID 排序后的可读序号区分，普通界面不展示裸项目 ID；未归属分组单独显示。
...
归档页的“删除本组归档”先请求服务端预览。预览展示项目名称、目标会话、权威数量和不可恢复提示；处理该分组全部已归档会话，搜索/分页不是授权来源。服务端冻结的清单不包含随后新增归档。设置页不提供删除批次历史入口；提交后关闭确认弹窗，成功提示实际删除数量，失败或清理未完成在页面保留可处理的告警，刷新后从内部记录找回。内部记录不删除、不迁移，不创建跨项目删除。
