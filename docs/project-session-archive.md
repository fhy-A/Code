...
归档页的“删除本组归档”先请求服务端预览。预览默认展示项目名称、整组权威数量和不可恢复提示，名单默认折叠。仅当当前搜索显示目标子集时，追加短句说明包含搜索结果之外的会话；不常驻长段快照/分页机制说明。处理的仍是该分组全部已归档会话，搜索/分页不是授权来源。服务端冻结的清单不包含随后新增归档。设置页不提供删除批次历史入口；提交后关闭确认弹窗，成功提示实际删除数量，失败或清理未完成在页面保留可处理的告警，刷新后从内部记录找回。内部记录不删除、不迁移，不创建跨项目删除。
