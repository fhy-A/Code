...
> 后续状态：H1-2 已将本模块接入服务端内存影子观察，H1-3 已在开发实例灰度启用显式事件版本并增加前端兼容读取，H1-4 已分离凭据严格拒绝与生产影子脱敏诊断并完成 H1 收口；正式实例仍默认写入旧格式。当前边界见 [`h1-4-shadow-credential-diagnostics.md`](h1-4-shadow-credential-diagnostics.md)。以下内容保留 H1-1 完成时的阶段事实。
