...
H3-1 先建立独立、确定性的离线回放入口，为后续状态机、任务契约和完成证据提供可重复验证地基。本阶段是在 H2-3 真实影子投影验收前提前完成的工具层准备，不代表 H2 已收口，也不代表完整 H3 门禁已经交付。
...
当前 H0 夹具共 15 条、106 个事件、17 个检查点和 4 个显式恢复点。H3-1 会验证全部 17 个检查点恢复和 4 个显式恢复点，但这些轨迹仍是 H0 时冻结的代表性基线，尚未等同于优化计划第六节所列 15 类最终完整场景。
...
1. 先完成 H2-3 开发实例真实轨迹验证并提取脱敏摘要，确认旧投影与规范投影的真实差异。
2. 将近期计时、刷新、工具组时序和非行动终止等真实问题转成最小脱敏 replay 夹具。
3. 补齐优化计划第六节仍缺的排队/并行、Child AgentRun、图片降级、手动压缩、上游错误和旧记录恢复等轨迹。
4. 关键轨迹稳定后再把 replay 接入发布门禁，并进入 H4 隔离浏览器 E2E。
