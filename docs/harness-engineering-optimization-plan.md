...
**阶段状态（2026-08-06）**：H4-1 已完成隔离浏览器基础设施、自检门禁，以及默认 bundle 纯文本、默认 bundle 单只读工具和 classic fallback 纯文本三条首批冒烟；详见 [`H4-1 浏览器 E2E 基础设施与首批冒烟`](harness/h4-1-browser-e2e-smoke.md)。刷新/重连等后续生命周期仍未完成，不得由首批冒烟推定通过。
