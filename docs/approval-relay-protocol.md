...
**发送成功不等于接收或开工**。接收方先按[接收回执短模板](#接收回执短模板)回执“接受 / 拒绝 / 阻塞”，附所见基线、范围冲突或缺失信息；接受仅表示理解该任务，实际写入仍须核对当前现场并取得/维护 canonical lease。无回执先只读核对投递和处理证据，不盲目重发或假定目标已开始。回执与交付包仅经已授权通道或人工转交返回，不因此产生自动发送权限。
...
默认各包一屏左右，细节放证据链接；人工包只汇总已有字段，**不替代原 v2 必填 envelope**，适用时原样附上完整 relay，并遵守 v1 恢复/去重。保留原标识，不造新 schema/编号；未知按上文对应边界处理，不把占位项当作已核验事实。本轮提供委托包、接收回执与交付包三份短模板；接收方按下方[接收回执短模板](#接收回执短模板)回应。

```text
协作类型 / 职责：委托执行或整体接手；来源保留什么、目标承担什么
来源：运行时 / 精确任务身份 / 角色
目标：运行时 / 精确任务身份 / 角色
授权与原指令：用户授权来源链接 / 源轮次 / 原标识（relayId、instructionKey 如适用）
任务 ID / 阶段：
原处理状态 / 回执证据：
基线与现场：准确 HEAD / tracked、cached、共享差量 / lease 状态
目标与允许范围：
验收 / 预期交付证据：
停止条件：
交付返回对象 / 指定复核者（按风险）：
...
接收方在**任何写入或开工前**先回执，三态择一：**接受 / 拒绝 / 阻塞**（拒绝或阻塞须写明原因与移交对象）。**接受仅表示理解任务，不取得写入权、不代表已开工**；写入仍须核对当前现场并按 [canonical lease](#角色与强制顺序) 取得与维护本人 lease。未知项标“未知”，不造字段、不改原标识，人工转交不扩大授权。

```text
对应任务 ID / 阶段 / 原指令标识：
接收者：运行时 / 精确任务身份（与任务包目标一致） / 角色
回执结论：接受 / 拒绝 / 阻塞（拒绝或阻塞写明原因与移交对象）
所见基线与现场：准确 HEAD / tracked、cached、共享差量 / lease 状态（含 holder）
处理状态核对：原指令是否已处理、已有回执或证据位置（未知标“未知”）
范围核对：允许范围、验收、停止条件；职责与授权来源是否清楚
冲突或缺失项：无 / 具体差异或缺失（含同标识不同内容的重复转交）
接受的含义：仅表示理解任务；不取得写入权、不代表已开工
lease：none / holder / leaseId；下一步（先只读核对 / 申请 lease 后执行 / 拒绝或阻塞的移交）
回执返回对象 / 指定复核者（按风险）：
详细合同与原始证据链接：
```
...
自检结果与指定复核结论分开写；有指定复核者时，执行完成不表示其已验收。通过已授权通道或人工原样转交，不因模板产生发送权限。

```text
对应任务 ID / 阶段 / 原指令标识：
来源执行者 / 目标接收者：运行时 / 精确任务身份
处理状态 / 对应回执位置：
完成结果与范围：
提交 / 未提交差量（含仓库外文件）：
验证：命令、环境、结果；失败 / skip / 未执行及原因
覆盖限制 / 原始证据链接（绑定提交或制品，核对冻结总数）：
最终 Git：HEAD / tracked、cached / 保留共享差量
lease：状态 / 本人持有或释放结果；临时状态：恢复结果 / 未恢复项
待复核问题 / 指定复核者与已有结论（无结论则明确待复核）：
下一步或停止点：
```
...
- **纯日志收口停止**：按风险完成所需技术复核后，纯粹记录该通过结论的日志提交只核对记录准确性、变更范围和最终 Git/lease；不重启完整审批，不为“复核日志通过”再次写日志或递归生成收口阶段。所需复核尚未完成时不能套用本规则；出现产品/规则实质变化、证据冲突或任一原有 stop condition，仍升级并按原门禁处理，不能借停止规则绕过复核或扩张提交/操作权限。
- **过程反馈**：只说明新增事实，以开始、真实阻塞、完成为主要节点；不重复播报未变化状态，仍遵守宿主要求的必要进度反馈。
