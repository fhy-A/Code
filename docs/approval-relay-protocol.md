...
- 正式发布完成、开发任务发生上下文压缩、历史已混入多个无关阶段，或审批任务无法仅靠 Git、开发日志、TODO 与相关 handoff 快速恢复现场时，当前阶段完成后停止向旧开发任务发送新 relay，旧任务停止作为 workspace writer。
- 已有用户任务创建授权时，由唯一审批 owner 创建或绑定新开发任务；缺少明确授权时只向用户建议轮换并等待确认，不得擅自创建。新任务重新核对当前 HEAD、工作树、cached、TODO、最新日志、相关 handoff 与 owner lease，然后用新的 relayId 申请 lease；不得复制旧任务的已完成历史、过期 ID 或受保护差量作为新事实源。
