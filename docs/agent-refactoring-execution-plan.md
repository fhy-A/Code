...
> **规范更新（2026-08-12）**：本阶段以下内容保留为早期架构草案。Goal 功能的后续产品与工程开发统一以 [《Code Goal 复杂长任务规划、执行与验收开发指导》](goal-mode-development-guide.md) 为准；冲突处已由“顶层步骤只使用 `pending / in_progress / completed` 三态、阻塞/失败/重试/等待用户仅作为内部运行信号、普通消息作为主要控制面、UI 不建设计划管理控制台”的确认决策取代。以下五态示例、预算字段和“暂停/继续/取消”交互不再是当前规范，不得直接据此实施。
...
每个阶段继续遵守项目的协作规范：一次只做一个阶段，先验证，涉及视觉或时序时等待人工确认，验收后更新当天开发日志、日志索引和 `TODO.md` 并独立提交。
