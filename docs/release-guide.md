...
| 2 | `git diff --check` → `node --check` / `py_compile` → 前端构建/freshness/语法 → `npm run verify:harness-replay`（30 秒）→ `pytest tests -v --tb=short --durations=30`（3600 秒） | 原11项共享检查全部执行一次，廉价失败先阻断耗时步骤 |
...
完整pytest使用已确认的3600秒（60分钟）上限，逐例实时输出nodeid与结果，失败使用短回溯，结束时报告最慢30项耗时；全部测试和断言保留，其他检查的超时不变。失败或超时即停止，不自动延长预算或重试prepare；新预算和命令参数进入共享定义指纹，旧凭证不能复用。
