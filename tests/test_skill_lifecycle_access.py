...
class TestCanonicalSkillAgentAccess(unittest.TestCase):
    ...
    def test_canonical_dependency_check_is_read_only_and_does_not_bind(self):
        run = self._create_run("dependency-read-only")
        result = self._call(
        ...
        self.assertNotIn("skillRuntimeBindings", server_mod._agent_run_record(run))

    def test_command_preflight_remains_fail_closed_without_canonical_binding(self):
        ...
        self.assertEqual(prepared["errorCode"], "skill_dependency_check_required")
