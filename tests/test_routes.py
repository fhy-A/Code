...
class TestHealthAndConfig(TestServerFixture):
    ...
    def test_agent_run_create_accepts_only_skill_selection_intent(self):
        ...
        second_skill_name = "observer-route-review"
        second_skill_dir = self._tmp_data / "skills" / second_skill_name
        second_skill_dir.mkdir(exist_ok=True)
        (second_skill_dir / "SKILL.md").write_text(
            f"---\nname: {second_skill_name}\ndescription: Route review observer\n"
            "tools: read_file\n---\n\nObserve review only.\n",
            encoding="utf-8",
        )
        (second_skill_dir / "evidence.json").write_text(json.dumps({
            "schemaVersion": 1,
            "requirements": [{
                "id": "review-input",
                "type": "tool_execution",
        ...
        with (
            ...
            status, data = _req("POST", "/api/agent/runs", json={
                ...
                "activeSkillName": "ignored-legacy-selection",
                "activeSkillNames": [skill_name, second_skill_name],
        ...
        self.assertEqual(
            [item["contractState"] for item in snapshot["skillEvidence"]["skills"]],
            ["valid", "valid"],
        )
        self.assertEqual(
            [item["name"] for item in snapshot["skillEvidence"]["activeSkills"]],
...
class TestFileTools(TestServerFixture):
    ...
    def test_registry_declares_read_interaction_and_proposal_effects(self):
        self.assertEqual(set(server_mod.SERVER_TOOL_REGISTRY), {
            ...
            "web_fetch", "use_skill", "check_skill_dependencies", "read_skill_resource", "save_memory", "delete_memory", "run_command",
        ...
        delegation = server_mod.SERVER_TOOL_REGISTRY["task"]
        self.assertEqual(delegation["effect"], "delegation")
