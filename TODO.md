...
- 继续寻找优质 DeepSeek 上游供应：比较稳定性、并发能力、延迟、工具调用兼容性、价格和结算条件；单独研究“高峰期 2 倍扣费”的适用时段、模型、Token 类型、倍率叠加顺序和上游成本影响，再决定是否及如何传导到 workbar 定价。
...
- 审计并优化 Codex / Claude Code 会话导入闭环（后端 + 前端）：检查扫描范围、重复导入与幂等性、项目和 `cwd` 归属、搜索/选择/批量导入步骤、进度反馈、部分失败、重试/取消、缓存刷新及导入完成后的无损列表更新；补齐真实样本和异常路径测试。
- 建立导入消息兼容矩阵并修复导入后兼容性：覆盖 Codex / Claude Code 的文本、reasoning、工具调用与结果、系统/摘要消息、附件、模型名、时间戳、usage 和异常 JSONL；验证导入会话在渲染、保存、继续对话、分支、压缩和再次加载后不会丢失、错序或退化。
...
- 明确区分顶部工具栏的“导入会话”和“导出会话”：重新设计不同轮廓与方向语义的图标，适当增大图标和点击辨识度，并检查悬停、提示文案、中英文和窄屏状态，避免当前两个小型文件夹箭头过于相似。
- 优化长会话时间线体验：将固定圆点改为 Codex 风格短线，默认保持低干扰，悬停时同时增加长度和提高颜色对比；一并检查长会话节点密度、滚动定位、提示内容、溢出和渲染性能。
